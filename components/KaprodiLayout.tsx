...
import KaprodiSidebar from './KaprodiSidebar';
...
export default function KaprodiLayout({ children, topbarTitle }: KaprodiLayoutProps) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!session || !user || user.role !== 'kaprodi') {
        router.push('/login');
      } else {
